...
- Check for a `Dockerfile` at the root, or a `dockhold.json` with `"build": {"dockerfile": "<path>"}`. Neither is needed for a stack Dockhold recognises on every account: Next.js with `output: 'standalone'` in `next.config.js`, Vite, Node with a `start` script and a `package-lock.json` and no `build` script, a static site with `index.html` at the root, FastAPI with `requirements.txt` naming `fastapi` and a root `main.py` defining `app`. Accounts with compute added also get automatic builds for any other stack. If the project is none of these, or the build declines it with "Could not build this automatically." and the named fix is more than one line, write a minimal `Dockerfile` for the stack using the patterns at https://dockhold.eu/docs/concepts/dockerfiles. One file, at the root, nothing else changes. When the decline names a one-line fix (add `output: 'standalone'`, add a `start` script, commit `package-lock.json`), make that change instead.
